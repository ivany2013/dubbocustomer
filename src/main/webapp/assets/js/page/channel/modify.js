...
$('#channelSubmit').click(function(){
	...
	$.ajax({
       type: "POST",
       url: "/article/system/channel/updateChannel",
       data: data,
        	...
        	$.msg("修改成功！");
         	setTimeout(function(){
         		...
         		if(mainFrame){
         			...
         			mainFrame.src="/article/system/channel/toChannelManage?websiteId="+websiteId+"&id="+id;
         		}
         		else
     			{
         			bootbox.alert("修改成功！mainFrame对象不存在，无法实现跳转");
     			}
         	},1000);
    	 }
         else{
				bootbox.alert("修改失败！status:"+result.status+"  msg:" + result.msg);
